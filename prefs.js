...
export default class GitHubTrayPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        const sortByRow = new Adw.ComboRow({
            ...
            model: Gtk.StringList.new([_('Last Updated'), _('Last Pushed'), _('Created Date'), _('Stars'), _('Name')]),
        });
        ...
        // Info about setting local paths
        const infoRow = new Adw.ActionRow({
            title: _('Setting Local Paths'),
            subtitle: _('Right-click on a repository in the tray menu to set or change its local path. Repositories with local paths will show a green border and folder icon.'),
        });
        localGroup.add(infoRow);
    ...
    _updateMappingsList(group, settings) {
        ...
        const rowsToRemove = [];
        while (child) {
            if (child instanceof Adw.ActionRow && child.get_title() !== 'Add New Mapping') {
